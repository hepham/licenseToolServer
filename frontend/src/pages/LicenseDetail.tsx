...
export default function LicenseDetail() {
  ...
  const handleDeactivate = async () => {
    if (!license || !confirm('Are you sure you want to deactivate this license? This will remove the device binding.')) return
    setDeactivating(true)
    try {
      await licenseApi.deactivate(license.id)
      setLicense({ ...license, status: 'inactive', device: undefined })
    } catch {
            ...
            {license.status !== 'revoked' && (
              <div className="flex gap-2">
                {license.status === 'active' && (
                  <button onClick={handleDeactivate} disabled={deactivating || revoking} className="btn-warning flex items-center gap-2">
                    {deactivating ? (
                      <>
                ...
                <button onClick={handleRevoke} disabled={revoking || deactivating} className="btn-danger flex items-center gap-2">
                  {revoking ? (
                    <>
                      <span className="loading-spinner" />
                      <span>Revoking...</span>
